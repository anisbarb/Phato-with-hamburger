...
Mobile-first React + TypeScript + Vite ride-share UI built with Leaflet, OSRM
routing, and Tailwind CSS. Two screens: Passenger and Driver.
...
The dev server reads `PORT` and `BASE_PATH` from the environment. With no env
vars set, Vite serves the app at `http://localhost:5173/`.
...
A `vercel.json` is included at the repository root that points Vercel at this
artifact:
...
Push the repository to GitHub and import it in Vercel — no manual project
...
- `src/lib/mockVehicles.ts` — replace `nearbyVehicles(center)` with your own
  driver feed (Supabase, your API, websockets, etc.)
